 ...
 *
 * Замена mail.tm. Изоляция между тестами достигается plus-addressing: каждый тест
 * получает уникальный адрес `<account>+qa_<rnd>@gmail.com`, все письма падают в общий
 * INBOX, а фильтрация идёт по получателю. Поэтому «token» здесь — это сам адрес-получатель
 * (в модели mail.tm токен идентифицировал изолированный ящик; в общем ящике его роль
 * играет адрес). Публичный интерфейс совпадает с прежним MailTmHelper — drop-in.
 *
...
export class GmailHelper {
  ...
  /** Совместимость с mail.tm-флоу: отдельный ящик создавать не нужно. */
  async createMailbox(): Promise<void> {
    // no-op: используется общий Gmail-ящик, изоляция — через plus-addressing.
  }
