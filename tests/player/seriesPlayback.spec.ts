...
// Since W3-2856 the canonical episode URL is `/video/{category}/{series-slug}/{episode-slug}`
// and the old 2-segment URL 308-redirects to it, so waits match by the episode-slug SUFFIX —
// that holds for both formats and for the transient pre-redirect URL.
const onEpisode = (slug: string) => (url: URL) => url.pathname.endsWith(`/${slug}`);
...
test('Series auto-advances to the next episode when an episode finishes', {
    ...
    await test.step('Finish episode 1 → episode 2 auto-advances and plays', async () => {
        ...
        await page.waitForURL(onEpisode(fx.episodeSlugs[1]), { timeout: 30_000 });
...
test('Selecting an episode in the under-player episodes popup switches playback to it', {
    ...
    const targetSlug = fx.episodeSlugs[targetEpisode - 1];
    ...
    await test.step(`Select episode ${targetEpisode} → player navigates to it and plays`, async () => {
        ...
        await page.waitForURL(onEpisode(targetSlug), { timeout: 30_000 });
        await page.waitForLoadState('domcontentloaded');
