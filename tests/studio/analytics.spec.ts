...
test('Analytics page displays seeded statistics', {
    ...
    await test.step('Switch chart to Subscribers and verify API data', async () => {
        ...
        const subsChartData = await analyticsPage.parseChartResponse(subsChartResponse);
    ...
    await test.step('Switch to "Last 7 days" and verify API response', async () => {
        ...
        // newSubscribers should reflect only subscribers within 7 days (day 0, 2, 4, 6 = 4)
        expect(analyticsData.newSubscribers, 'New subscribers in last 7 days should be 4').toBe(4);
